...
If you’re trying to send to your own wallet, you’ll see a different warning: *The receiving address can’t be the same as the sending address.*
...
1. **Wait** — some networks take time to confirm transactions.
1. **Restart the app** — this forces a fresh connection to the blockchain.
...
1. If you’re using a VPN or firewall, try disabling it — some networks block blockchain RPC connections.
1. Wait a few minutes and try again — the issue may be temporary.
1. Restart the app.
...
1. **Update the app** — make sure you’re using the latest version.
1. **Reinstall the app** — your wallets will not be lost if you have your seed phrase or cloud backup. Restore them after reinstalling.
...
1. **Refresh the balance** — pull down on the main screen to reload your token list.
1. **Check for multiple accounts** — when importing a seed phrase, the app can detect multiple accounts. Make sure you’ve added all the ones you need.
